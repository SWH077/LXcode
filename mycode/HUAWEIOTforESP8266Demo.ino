...
#define WIFI_SSID "SSWH"    //wifi名称
#define WIFI_PASSWORD "As123456"  //wifi密码
//MQTT三元组
#define ClientId "shiweihao1"
#define Username "bkrc"
#define Password "88888888"

#define MQTT_Address "115.28.209.116"
#define MQTT_Port 1883
#define Iot_link_MQTT_Topic_Report "device/991bcb2ead375c3c/up"
#define Iot_link_MQTT_Topic_Commands "device/991bcb2ead375c3c/down"
...
void protocolJSON(unsigned char JsonData[]) {
  ...
  root["sign"] = "991bcb2ead375c3c";
  root["type"] = 3;

  JsonObject& Temp_Humi = data.createNestedObject("Temp_Humi");            //温湿度传感器模块标识
  JsonObject& Fire = data.createNestedObject("Fire"); 
  JsonObject& Vec = data.createNestedObject("Vec");
  JsonObject& out = data.createNestedObject("out_heat"); 
  JsonObject& Air = data.createNestedObject("Air");   
  sprintf(printf_buf, "%d", 0);

  Temp_Humi["Temp"] = printf_buf;  //温度模型标识
  Temp_Humi["Humi"] = printf_buf;  //湿度模型标识
  Fire["fire"]=printf_buf; 
  Vec["vec"]=printf_buf;
  out["heat"]=printf_buf;
  Air["air"]=printf_buf;

    if (JsonData[0] == 0xAA) {
      sprintf(total_data, "%d", JsonData[1]);
      Temp_Humi["Temp"] = total_data;
      memset(total_data, 0, sizeof(total_data));
      delay(50);

      sprintf(total_data, "%d", JsonData[2]);
      Temp_Humi["Humi"] = total_data;
      memset(total_data, 0, sizeof(total_data));
      delay(50);

      sprintf(total_data, "%d", JsonData[3]);
      Fire["fire"] = total_data;
      memset(total_data, 0, sizeof(total_data));
      delay(50);
      ...
      sprintf(total_data, "%d", JsonData[6]);
      Air["air"] = total_data;
      memset(total_data, 0, sizeof(total_data));
      delay(50);
    }
